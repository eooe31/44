...
def gen_user(choice):
    ...
    if choice == "سداسيات":
        ...
        c = random.choices(a)
        d = random.choices(a)
        f = [c[0], "888", "ajeisudheox", c[0], d[0], "hwud", d[0]]
        random.shuffle(f)
        username = "".join(f)
    else:
        return "error"
...
**-- -- -- -- -- -- -- -- --
 ...
 -- -- -- -- -- 
 سداسي
 sxxsxx - s55sss - ss8888
  -- -- -- -- --  
  ثلاثي 
  s_x_e - s_x_6 - s_6_x - s_5_7
 -- -- -- -- --
 بوت
 sxcbot - xc6bot - s6cbot
 -- -- -- -- -- 
 ارقام
 s777x - xc777
 -- -- -- -- -- 
...
async def hunterusername(event):
    ...
    isclaim.clear()
    isclaim.append("off")
    await event.client.send_message("@knnnk", "**- انتهاء الفحص تم الصيد -**")
